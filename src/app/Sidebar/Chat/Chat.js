...
class Chat extends Component {
  ...
  componentDidMount() {
    ...
    window.socket.on('joined', (data) => {
      ...
      if (data.type == 'game' && !tabs.find(item => {
        ...
        if (this._ismounted) self.setState({
          tabs: tabs,
          activeTab: data.name
        });
      }
      if (data.type == 'direct message' && !tabs.find(item => {
          return data.name == item.name
        })) {
        tabs.push({
          name: data.name.split(' to ')[1],
          isVisible: true,
          socketName: data.name,
          action: 'message to',
          messages: []
    ...
    window.socket.on('close room', (data) => {
      let tabs = this.state.tabs.filter(tab => {
        return tab.socketName != data.room
      });

      tabs.find(tab => {
        return tab.socketName == 'general'
  ...
  activateTab = (tabName) => {
    if (this.state.pendingMessages.indexOf(tabName) > -1) {
      this.setState({
        pendingMessages: this.state.pendingMessages.filter(tab => {
          tab != tabName
        }),
        activeTab: tabName
      });
  ...
  onSendMessage = (text) => {
    let tab = this.state.tabs.find(tab => {
      return tab.socketName == this.state.activeTab
    })
    if (tab.socketName == 'general') {
      window.socket.emit(tab.action, text)
    } else {
      window.socket.emit(tab.action, {
        text: text,
        room: tab.socketName
      });
    }
  ...
  closeTab = (tabName) => {
    ...
    changedTabs.find(tab => {
      return tab.socketName == tabName
    }).isVisible = false;
    if (this._ismounted) this.setState({
      activeTab: 'general',
      tabs: changedTabs
